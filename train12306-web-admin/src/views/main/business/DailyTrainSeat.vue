...
<!--        </a-button>-->
    ...
    >
      ...
      >
        ...
        <a-form-item name="sell" label="售卖情况">
          <a-input v-model:value="formData.sell" placeholder="请输入售卖情况" />
        </a-form-item>
      </a-form>
    </a-modal>
  </div>
...
const rules = {
  ...
  sell: [
    {required: true, message: '请输入售卖情况', trigger: 'change'}
  ],
};
...
const handleOk = () => {
  formRef.value.validate().then(() => {
    ...
    axios.post('/business/admin/daily-train-seat/saveDailyTrainSeat', formData.value)
        .then(res => {
          if (res.data.code === 200) {
            ...
            formData.value = {}
            visible.value = false;
            listDailyTrainSeatPage(pagination.current, pagination.pageSize)
          } else {
...
// 分页查询
const listDailyTrainSeatPage = (pageNum, pageSize) => {
  loading.value = true;
  axios.get('/business/admin/daily-train-seat/listDailyTrainSeatPage', {
...
const handleTableChange = (page) => {
  listDailyTrainSeatPage(page.current, page.pageSize)
}
// 刷新
const handleRefresh = () => {
  listDailyTrainSeatPage(1, pagination.pageSize)
}
// 编辑
...
const handleDelete = (record) => {
  axios.delete(`/business/admin/daily-train-seat/deleteById/${record.id}`).then(res => {
    if (res.data.code === 200) {
