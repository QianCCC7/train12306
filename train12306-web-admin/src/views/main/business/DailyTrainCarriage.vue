...
const handleOk = () => {
  formRef.value.validate().then(() => {
    ...
    axios.post('/business/admin/daily-train-carriage/saveDailyTrainCarriage', formData.value)
        .then(res => {
          if (res.data.code === 200) {
            ...
            formData.value = {}
            visible.value = false;
            listDailyTrainCarriagePage(pagination.current, pagination.pageSize)
          } else {
...
// 分页查询
const listDailyTrainCarriagePage = (pageNum, pageSize) => {
  loading.value = true;
  axios.get('/business/admin/daily-train-carriage/listDailyTrainCarriagePage', {
...
const handleTableChange = (page) => {
  listDailyTrainCarriagePage(page.current, page.pageSize)
}
// 刷新
const handleRefresh = () => {
  listDailyTrainCarriagePage(1, pagination.pageSize)
}
// 编辑
...
const handleDelete = (record) => {
  axios.delete(`/business/admin/daily-train-carriage/deleteById/${record.id}`).then(res => {
    if (res.data.code === 200) {
