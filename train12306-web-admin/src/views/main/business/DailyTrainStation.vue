...
const handleOk = () => {
  formRef.value.validate().then(() => {
    ...
    axios.post('/business/admin/daily-train-station/saveDailyTrainStation', formData.value)
        .then(res => {
          if (res.data.code === 200) {
            ...
            formData.value = {}
            visible.value = false;
            listDailyTrainStationPage(pagination.current, pagination.pageSize)
          } else {
...
// 分页查询
const listDailyTrainStationPage = (pageNum, pageSize) => {
  loading.value = true;
  axios.get('/business/admin/daily-train-station/listDailyTrainStationPage', {
...
const handleTableChange = (page) => {
  listDailyTrainStationPage(page.current, page.pageSize)
}
// 刷新
const handleRefresh = () => {
  listDailyTrainStationPage(1, pagination.pageSize)
}
// 编辑
...
const handleDelete = (record) => {
  axios.delete(`/business/admin/daily-train-station/deleteById/${record.id}`).then(res => {
    if (res.data.code === 200) {
